...
// Function to clear previous landmarks from DOM
function clearLandmarks() {
  const existingLandmarks = document.querySelectorAll('.landmark');
  existingLandmarks.forEach((landmark) => landmark.remove());
}

// Function to position landmarks on the canvas
function positionLandmarkDiv(landmark, id, className, text) {
  const landmarkDiv = document.getElementById(id) || document.createElement('div');
  landmarkDiv.id = id;
  ...
  if (isRecording) {
    recordingData.push({ id, x: adjustedX, y });
  }
}

// Function to handle results from the hand model
function onResults(results) {
  if (!animationTriggered) {
    trackedPositions = []; // Reset tracked positions when starting a new tracking session
...
// Function to generate the CSS animation from recorded data
...
// Function to start recording the landmark positions
...
// Function to stop recording and trigger animation
function stopRecording() {
  isRecording = false;
  console.log('Recording stopped...');
  
  // Create a new div to display the animation
  animationDiv = document.createElement('div');
  animationDiv.classList.add('animation-container');
  document.body.appendChild(animationDiv);
  
  // Generate and apply the animation to the new div
  generateCSSAnimation();
  
  // Add the landmark divs to the new container
  recordingData.forEach((data) => {
    const div = document.getElementById(data.id);
    if (div) {
      animationDiv.appendChild(div); // Append the divs to the new container
    }
  });
}
