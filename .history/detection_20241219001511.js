...
function positionLandmarkDiv(landmark, id, className, text) {
  ...
  trackedPositions.push({ id, x: adjustedX, y }); // Store position for animation
}
...
function onResults(results) {
  ...
  canvasCtx.scale(-1, 1); // Horizontal flip
  canvasCtx.translate(-canvasElement.width, 0); // Correct translation
...
// Capture tracked positions after some time and turn them into a CSS animation

function generateCSSAnimation() {
  const keyframes = trackedPositions.map((position, index) => {
    return `${index * 0.1}s { 
      #${position.id} {
  ...
  const animationDuration = `${trackedPositions.length * 0.1}s`;
  ...
  // Insert the CSS animation into a <style> element in the head
  const style = document.createElement('style');
  style.textContent = cssAnimation;
  document.head.appendChild(style);
}

// Call this function once to create the animation from recorded data
setTimeout(generateCSSAnimation, 5000); // Record positions for 5 seconds
