...
let motionData = [];  // Array to hold the motion data
let recordingInProgress = false;  // Flag to control when to record the animation
...
// Function to position a div at a given landmark position with unique class
function positionLandmarkDiv(landmark, id, className, text, timestamp) {
  if (!recordingInProgress) return;  // Skip if we're not recording
  ...
  // Store the position data in the motionData array with timestamp
  motionData.push({
    id: id,
    x: adjustedX,
    y: y,
    timestamp: timestamp,  // Ensure timestamp is captured
    className: className,
    text: text,
  });
...
// Function to handle the results from the model
function onResults(results) {
  if (!recordingInProgress) return;  // Skip if recording is done
  ...
  const timestamp = performance.now(); // Record current time for motion data
  ...
  if (results.multiHandLandmarks) {
    ...
    results.multiHandLandmarks.forEach((landmarks, handIndex) => {
      ...
      const wristDivId = `wrist-div-${handIndex}`;
      positionLandmarkDiv(wrist, wristDivId, `wrist-${handIndex}`, "f", timestamp);

      // Handle MCP points
      const mcp1 = landmarks[1];
      const mcpDivId1 = `mcp-div-${handIndex}-1`;
      positionLandmarkDiv(mcp1, mcpDivId1, `mcp-${handIndex}-1`, "a", timestamp);

      const mcp2 = landmarks[5];
      const mcpDivId2 = `mcp-div-${handIndex}-2`;
      positionLandmarkDiv(mcp2, mcpDivId2, `mcp-${handIndex}-2`, "b", timestamp);

      const mcp3 = landmarks[9];
      const mcpDivId3 = `mcp-div-${handIndex}-3`;
      positionLandmarkDiv(mcp3, mcpDivId3, `mcp-${handIndex}-3`, "c", timestamp);

      const mcp4 = landmarks[13];
      const mcpDivId4 = `mcp-div-${handIndex}-4`;
      positionLandmarkDiv(mcp4, mcpDivId4, `mcp-${handIndex}-4`, "d", timestamp);

      const mcp5 = landmarks[17];
      const mcpDivId5 = `mcp-div-${handIndex}-5`;
      positionLandmarkDiv(mcp5, mcpDivId5, `mcp-${handIndex}-5`, "e", timestamp);

      // Handle finger tip points
      const tip1 = landmarks[4];
      const tipDivId1 = `tip-div-${handIndex}-1`;
      positionLandmarkDiv(tip1, tipDivId1, `tip-${handIndex}-1`, "1", timestamp);

      const tip2 = landmarks[8];
      const tipDivId2 = `tip-div-${handIndex}-2`;
      positionLandmarkDiv(tip2, tipDivId2, `tip-${handIndex}-2`, "2", timestamp);

      const tip3 = landmarks[12];
      const tipDivId3 = `tip-div-${handIndex}-3`;
      positionLandmarkDiv(tip3, tipDivId3, `tip-${handIndex}-3`, "3", timestamp);

      const tip4 = landmarks[16];
      const tipDivId4 = `tip-div-${handIndex}-4`;
      positionLandmarkDiv(tip4, tipDivId4, `tip-${handIndex}-4`, "4", timestamp);

      const tip5 = landmarks[20];
      const tipDivId5 = `tip-div-${handIndex}-5`;
      positionLandmarkDiv(tip5, tipDivId5, `tip-${handIndex}-5`, "5", timestamp);
    });
  }
...
// Function to start recording the animation data
function startRecording() {
  motionData = [];  // Clear any previous data
  recordingInProgress = true;
  console.log('Recording started...');
}

// Function to stop recording after a certain time
function stopRecording() {
  recordingInProgress = false;
  console.log('Recording finished. Total frames:', motionData.length);
  // You can now use the motionData array for playback
}

// Function to play back the recorded animation
function playAnimation() {
  let startTime = null;

  // Function to interpolate and animate motion
  function animate(timestamp) {
    if (!startTime) startTime = timestamp;
    const progress = (timestamp - startTime) / 1000; // Calculate the progress (in seconds)

    // Loop through the recorded motion data and apply interpolated positions
    motionData.forEach(({ id, x, y, timestamp: recordTime }) => {
      if (recordTime && timestamp >= recordTime) { // Ensure recordTime exists
        const landmarkDiv = document.getElementById(id);
        if (landmarkDiv) {
          const xProgress = x;  // Interpolate x if needed
          const yProgress = y;  // Interpolate y if needed
          landmarkDiv.style.transform = `translate(${xProgress}px, ${yProgress}px)`;
        }
      }
    });

    // Continue the animation until all data is played back
    if (progress < motionData[motionData.length - 1].timestamp) {
...
// Example of starting and stopping recording:
startRecording();

// Stop the recording after 5 seconds (for demonstration)
setTimeout(stopRecording, 5000);

// Play the animation after recording is finished
